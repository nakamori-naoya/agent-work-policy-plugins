...
POLICY_SCHEMA = {
    "version": int,
    "workspace.use_worktree": bool,
    "workspace.require_clean_start": bool,
    "workspace.base_branch": str,
    "workspace.branch_prefix": str,
    "workspace.worktree_root": str,
    "git.remote": str,
    "permissions.commit": bool,
    "permissions.push": bool,
    "permissions.pull_request": bool,
    "permissions.merge": bool,
    "gates.before_commit": bool,
    "gates.before_push": bool,
    "gates.before_pull_request": bool,
    "gates.before_merge": bool,
...
def validate_policy(cfg, path):
    """policy fileのkey集合と型をschemaと完全一致で検査する。fallbackも既定値も持たない。"""
    if not isinstance(cfg, dict):
        emit({"error": "policy設定がmappingではない", "config": path}, 2)
    leaves = dict(leaf_paths(cfg))
    unknown = sorted(set(leaves) - set(POLICY_SCHEMA))
    missing = sorted(set(POLICY_SCHEMA) - set(leaves))
    if unknown or missing:
        emit({"error": "policy設定のkeyがschemaと一致しない", "config": path, "unknown": unknown, "missing": missing}, 2)
    for key, expected in POLICY_SCHEMA.items():
        value = leaves[key]
        if expected is bool and type(value) is not bool:
            emit({"error": "policy設定の型が不正", "config": path, "key": key, "expected": "boolean"}, 2)
        if expected is int and (type(value) is not int):
            emit({"error": "policy設定の型が不正", "config": path, "key": key, "expected": "integer"}, 2)
    ...
    for key in ("workspace.base_branch", "workspace.branch_prefix", "git.remote", "merge.method"):
        if not leaves[key]:
            emit({"error": "policy設定の値が空", "config": path, "key": key}, 2)
    if cfg["merge"]["method"] not in MERGE_METHODS:
        emit({"error": "merge.methodが不正", "config": path, "allowed": list(MERGE_METHODS)}, 2)
    if cfg["merge"]["method"] == "fast-forward" and not cfg["merge"]["delete_branch"]:
        emit({"error": "fast-forwardではmerge.delete_branch: trueが必須", "config": path}, 2)
    if cfg["merge"]["delete_worktree"] and not cfg["workspace"]["use_worktree"]:
...
def load_config(path):
    """対象repositoryの policy file（<repo>/.harness-plugins/agent-work-policy.config.yml）を直接読む。"""
    policy = Path(path)
    if policy.is_symlink() or not policy.is_file():
        emit({"error": "policy設定fileが無い", "config": str(policy), "reason": "policy_missing"}, 2)
    proc = command(["yq", "-o=json", "-I=0", ".", str(policy)])
    if proc.returncode:
        emit({"error": "policy設定を読めない", "config": str(policy), "detail": proc.stderr.strip()}, 2)
    try:
        cfg = json.loads(proc.stdout)
    except json.JSONDecodeError as exc:
        emit({"error": "policy設定がJSONへ変換できない", "config": str(policy), "detail": str(exc)}, 2)
    validate_policy(cfg, str(policy))
    cfg["_policy_path"] = str(policy.resolve())
    return cfg
...
def bound_repo_root(cfg, repo):
    """policy fileは対象repository rootの POLICY_FILE でなければならない。別repositoryの設定を流用させない。"""
    root = repo_root(repo)
    configured = str(Path(root, POLICY_FILE).resolve())
    if cfg.get("_policy_path") != configured:
        emit({
            "error": "設定と対象repositoryが一致しない",
            "config": cfg.get("_policy_path"),
            "expected_config": configured,
            "repo_root": root,
        }, 2)
    return root
...
def safe_paths(root, paths_file):
    ...
    if any(item != item.strip() for item in raw):
        emit({"error": "commit対象pathの前後空白は許可されない"}, 2)
    paths = raw
