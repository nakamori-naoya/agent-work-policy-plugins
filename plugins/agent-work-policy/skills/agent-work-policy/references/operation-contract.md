...
この入口のagentが実行するのは `scripts/invoke.py` だけである。`control.py` を呼ぶのは `invoke.py` だけで、外部pluginは公開playbookを通してしか操作を要求できない。内部の引数、状態名、終了code、公開の語彙への写し方は、両scriptの冒頭のコメントが持つ。
